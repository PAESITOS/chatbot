...
def google_accuracy(vision_client,image,real_label):
    """ 
    This function calculates the accuracy given by the google vision service
        by one image. We valorate relevant information. 
    """
    keys = google_words.keys()
    ...
    for label in labels:
        counter += 1
        if real_label == label.description:
            pos = counter
            score=(11-counter)/10
        else:
    ...
    for path in data_directory.iterdir():
        if path.is_dir():
            for ingredient in path.glob('*.jpg'):
                ...
                s_google,p_google = google_accuracy(vision_client,image,real_label)
                ...
                if s_google > s_amazon:
                    results['punt_google']+=1
                elif s_google < s_amazon:
                    results['punt_amazon']+=1
                results['pos_google'].append(p_google)
                results['pos_amazon'].append(p_amazon)
                results['score_google'].append(s_google)
...
def main():
    ...
    key_path = str(Path.cwd())+"/apikey.json"
